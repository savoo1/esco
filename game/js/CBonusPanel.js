function CBonusPanel() {
  ...
  this._init = function () {
    ...
    _oMultAmountText = new createjs.Text(
      formatEntries(0),
      "56px " + FONT_GAME_1,
      "#ffba00"
    );
  ...
  this._loadAllResources = function () {
    ...
    s_oSpriteLibrary.addSprite("clouds", "./sprites/bonus/clouds.webp");
    s_oSpriteLibrary.addSprite("jar", "./sprites/bonus/jar.webp");
    for (var k = 0; k < 30; k++) {
      s_oSpriteLibrary.addSprite(
        "jar_anim_" + k,
        "./sprites/bonus/jar_anim/jar_anim_" + k + ".webp"
      );
    }
    ...
    for (var i = 0; i < 32; i++) {
      s_oSpriteLibrary.addSprite(
        "thunderbolt_" + i,
        "./sprites/bonus/thunderbolt/thunderbolt_" + i + ".webp"
      );
    }

    s_oSpriteLibrary.addSprite("column_0", "./sprites/bonus/column_0.webp");
    s_oSpriteLibrary.addSprite("column_1", "./sprites/bonus/column_1.webp");
    s_oSpriteLibrary.addSprite("column_2", "./sprites/bonus/column_2.webp");
    s_oSpriteLibrary.addSprite("column_3", "./sprites/bonus/column_3.webp");
    for (var j = 0; j < 101; j++) {
      s_oSpriteLibrary.addSprite(
        "zeus_animations_" + j,
        "./sprites/bonus/zeus_animations/zeus_animations_" + j + ".webp"
      );
    }
  ...
  this._startBonus = function () {
    ...
    createjs.Tween.get(_oZeus, { loop: true, bounce: true }).to(
      { y: 336 },
      4000,
  ...
  this._destroyJar = function () {
    ...
    _oScoreText = new CScoreText(
      formatEntries(_iCurMult /**_iCurBet*/),
      _aButtons[_iCurColumns].getX(),
      _aButtons[_iCurColumns].getY(),
      _oContainer
    );
    this.refreshScoreAmount();

    setTimeout(function () {
      _aButtons[_iCurColumns].tweenUp(CANVAS_HEIGHT, 500);
    }, 1500);
    setTimeout(function () {
